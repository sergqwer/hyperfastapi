...
Проєкт `hyperfastapi` переписує FastAPI на Rust + PyO3 зі збереженням Python-сумісного API. Перед написанням Rust-коду нам потрібно зафіксувати "істину" про поведінку оригінального FastAPI у формі тестів, щоб:
